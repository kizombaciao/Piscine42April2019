...
int valid_brackets(char *str)
{
	...
	{
		c = str[i]; // can we assume that 1st element is a bracket???
		if (c == '(' || c == '[' || c == '{')
		{
			index += 1;
			buff[index] = c;
		...
		{
			printf("index: %d\n", index);
			if (index < 0 || check(buff[index], c) == 0) // why index < 0 ???
				continue ;			
			matches[c] += 1;
			buff[index] = '\0';
			index -= 1;
...
// will be skipped and next iteration of the loop will begin.